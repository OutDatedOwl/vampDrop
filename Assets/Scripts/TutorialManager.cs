...
namespace Vampire.DropPuzzle
{
    ...
    public class TutorialManager : MonoBehaviour
    {
        ...
        private void CompleteTutorial()
        {
            ...
            // Re-enable the day/night cycle and start fresh from Day
            if (cycleManager != null)
            {
                cycleManager.enabled = true;
                cycleManager.isPaused = false;
                cycleManager.ForcePhase(DayNightCycleManager.TimeOfDay.Day);
                // Debug.Log("[TutorialManager] Day/Night cycle ENABLED — starting from Day");
            }

            // Persist tutorial completion so puzzle loader knows to skip puzzle 0
